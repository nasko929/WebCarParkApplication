...
@WebServlet(name = "StartDrivingServlet", urlPatterns = {"/StartDrivingServlet"})
        ...
        if (idOfChosenCar.matches("^\\d+$")) {
            try {
                ...
                if (resultSet.next()) {
                    ...
                    if (car.isStartedEngine() && car.getCurrentFuel() >= 5 && !car.isBeingDriven()) {
                        car.setBeingDriven(true);
                        car.setCurrentFuel(car.getCurrentFuel() - 5);
                        carOperations.updateData(car, id);
                    } else if(car.isBeingDriven()) {
                        request.setAttribute("error","The car is already being driven.");
                    } else if (!car.isStartedEngine()) {
                        request.setAttribute("error", "The car's engine is not started!");
                    } else if(car.getCurrentFuel() < 5) {
                        request.setAttribute("error","Not enough fuel to drive.");
                    }
